...
import { updateOrderStatus, deleteOrder, getOrderById } from "@/lib/actions/order-actions"
interface Props {
    order :NonNullable< Awaited<ReturnType<typeof getOrderById>>>
}

export function OrderDetailsAdmin({ order }: Props) {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({
          variant: "destructive",
          title: "Erreur",
          description: result.message || "Une erreur est survenue lors de la mise à jour du statut.",
        })
      }
    } catch (error) {
      toast({
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({
          variant: "destructive",
          title: "Erreur",
          description: result.message || "Une erreur est survenue lors de la suppression de la commande.",
        })
      }
    } catch (error) {
      toast({
